  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        // Position-based color gradient: pure white center -> soft white edges
        float normDist = clamp(distanceFromCenter / 8.0, 0.0, 1.0);
        vec3 c0 = vec3(1.00, 1.00, 1.00); // pure white (center)
        vec3 c1 = vec3(0.90, 0.92, 0.95); // cool white (mid)
        vec3 c2 = vec3(0.75, 0.78, 0.85); // slightly dimmer white (edge)
        vec3 particleColor = mix(c0, c1, smoothstep(0.0, 0.45, normDist));
        particleColor = mix(particleColor, c2, smoothstep(0.4, 1.0, normDist));
